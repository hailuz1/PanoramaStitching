...
class Solution:
    ...
    def compute_homography_naive(match_p_src: np.ndarray,
        ...
        for i in range(0,n):
            ...
            x_val = match_p_src[0][i]
            y_val = match_p_src[1][i]
        
            u_val= match_p_dst[0][i]
            v_val= match_p_dst[1][i]
            
            a_mat[mat_index] = [x_val, y_val, 1, 0, 0, 0, -u_val * x_val, -u_val * y_val, -u_val]
            a_mat[mat_index+1]= [0, 0, 0, x_val, y_val, 1, -v_val*x_val, -v_val*y_val, -v_val]
            
            mat_index = mat_index + 2
    
        a_mat = np.asarray(a_mat)

        U, S, Vh = np.linalg.svd(a_mat)
        norm = Vh[-1, :] / Vh[-1, -1] # Taking the last row and normalize it
        H= np.reshape(norm, (3,3))
        return H
    ...
    def compute_forward_homography_slow(
        ...
        row = dst_image_shape[0]
        col = dst_image_shape[1]
        dst_image = np.zeros((row, col,3))
        for i in range(src_image.shape[1]):
            for j in range(src_image.shape[0]):
                result = np.dot(homography, [i, j, 1])
                i_dst, j_dst, _ = (result / result[2] + 0.5).astype(int)
                if i_dst >= 0 and i_dst < col:
                    if j_dst>= 0 and j_dst < row:
    ...
    def compute_forward_homography_fast(
        ...
        col = dst_image_shape[1]
        dst_image = np.zeros((row, col, 3))
        [y,x]= np.meshgrid(np.linspace(0,src_image.shape[0]-1,src_image.shape[0]),np.linspace(0,src_image.shape [1]-1,src_image.shape [1]))
        x = x.T
        y = y.T
        xx = x.reshape(1,src_image.shape[0]*src_image.shape[1])
        yy = y.reshape(1,src_image.shape[0]*src_image.shape[1])
        one_v = np.ones((1,src_image.shape[0]*src_image.shape[1]))
        mat_src = np.vstack([xx,yy,one_v])
        dst_coor = np.matmul(homography,mat_src)
        dst_coor /= dst_coor[2,:] 
        
        dst_x = np.round(dst_coor[0,:])
        dst_y = np.round(dst_coor[1,:])
        ind_clip = np.argwhere((dst_x >= 0) & (dst_x < col) & (dst_y >= 0) & (dst_y < row))

        dst_image[dst_y[ind_clip].astype(int),dst_x[ind_clip].astype(int),:] = src_image[mat_src[1,ind_clip].astype(int),mat_src[0,ind_clip].astype(int),:]
        dst_image = dst_image.astype(np.uint8)
    ...
    def test_homography(homography: np.ndarray,
        ...
        est = np.matmul(homography, xy)
        est = np.round(est / est[2, :])
        err = est - uv
        norms=np.zeros((1,err.shape[1]))
        for i in range(err.shape[1]):
             norms[0,i] = np.array(np.linalg.norm(err[:2, i]))
        function = np.vectorize(lambda x: x < max_err)
        fit_points = function(norms)
        
        fit_percent = np.mean(fit_points)
        dist_mse = np.mean(norms[fit_points])
        if np.isnan(dist_mse):
            dist_mse = 10 ** 9
        return fit_percent, dist_mse
    ...
    def meet_the_model_points(homography: np.ndarray,
        ...
        function = np.vectorize(lambda x: x < max_err)
        fit_points = function(norms)
        return match_p_src[:, fit_points.flatten()], match_p_dst[:, fit_points.flatten()]
    ...
    def compute_homography(self,
        ...
        fit_percent_terget = 0.5 # = d
        n = match_p_src.shape[1]
        # n = 4
        p = 0.99
        k = int(np.ceil(np.log(1 - p) / np.log(1 - inliers_percent ** 4)))
        best_mse = np.inf
        best_mse_temp = np.inf

        for i in range(k):
            # Randomaly select n points
            ind_p = np.random.randint(1, n, 4)
            # Compute the model using the n points
            H_naive = self.compute_homography_naive(match_p_src[:, ind_p], match_p_dst[:, ind_p])
            # Find inliers
            fit_percent,_ = self.test_homography(H_naive, match_p_src, match_p_dst, max_err)
            mp_src_fit,mp_dst_fit = self.meet_the_model_points(H_naive, match_p_src, match_p_dst, max_err)
 
            H_naive_fit_point = self.compute_homography_naive(mp_src_fit,mp_dst_fit)
            _, dist_mse_fit_point = self.test_homography(H_naive_fit_point, mp_src_fit,mp_dst_fit, max_err)
            
            if dist_mse_fit_point < best_mse:
                if dist_mse_fit_point < best_mse_temp:
                    best_H_temp = H_naive_fit_point
                    best_mse_temp = dist_mse_fit_point

                if fit_percent > fit_percent_terget:
                    best_H = H_naive_fit_point
                    best_mse = dist_mse_fit_point
        try:
            best_H
        except:
            print('Please run again.\nThere is no good H for these points under the conditions required.\nFor better results you can change p to be 0.999')
    ...
    def compute_backward_mapping(
        ...
        row = dst_image_shape[0]
        col = dst_image_shape[1]
        src_backward_image = np.zeros((row, col, 3))
        [y,x]= np.meshgrid(np.linspace(0,row-1,row),np.linspace(0,col-1,col))
        x = x.T
        y = y.T
        xx = x.reshape(1,row*col)
        yy = y.reshape(1,row*col)
        one_v = np.ones((1,row*col))
        mat_dst = np.vstack([xx,yy,one_v])
        src_coor = np.matmul(backward_projective_homography,mat_dst)
        src_coor /= src_coor[2,:] 
    
        src_x = src_coor[0,:]
        src_y = src_coor[1,:]
        ind_clip = np.argwhere((src_x >= 0) & (src_x < src_image.shape[1]) & (src_y >= 0) & (src_y < src_image.shape[0]))
        [x,y]= np.meshgrid(np.linspace(0,src_image.shape[1]-1,src_image.shape[1]),np.linspace(0,src_image.shape[0]-1,src_image.shape[0]))
        x_in_img = src_x[ind_clip]
        y_in_img = src_y[ind_clip]

        val = src_image[y.reshape(src_image.shape[1]*src_image.shape[0],1).astype(int),x.reshape(src_image.shape[1]*src_image.shape[0],1).astype(int),:]
        points = np.hstack([x.reshape(src_image.shape[1]*src_image.shape[0],1),y.reshape(src_image.shape[1]*src_image.shape[0],1)])
        grid_z0z = griddata(points,val,(x_in_img,y_in_img),method='cubic')

        src_backward_image[mat_dst[1,ind_clip].astype(int),mat_dst[0,ind_clip].astype(int),:] = grid_z0z[:,0,:,:]
        src_backward_image = np.clip(src_backward_image, 0, 255).astype(np.uint8)

        return src_backward_image
    ...
    def add_translation_to_backward_homography(backward_homography: np.ndarray,
        ...
        translation_matrix = np.diag([1,1,1])
        translation_matrix[0,2] = -pad_left
        translation_matrix[1,2] = -pad_up

        Final_H = np.matmul(backward_homography,translation_matrix)
        final_homography = Final_H/np.linalg.norm(Final_H)
    ...
    def panorama(self,
        ...
        forward_H = self.compute_homography(match_p_src, match_p_dst, inliers_percent, max_err)
        panorama_rows_num, panorama_cols_num, pad_struct = self.find_panorama_shape(src_image, dst_image, forward_H)
        backward_H = np.linalg.inv(forward_H)
        backward_H = backward_H/backward_H[-1,-1]
        Final_H = self.add_translation_to_backward_homography(backward_H,pad_struct[3],pad_struct[0])
        src_image_after_backward = self.compute_backward_mapping(Final_H, src_image, (panorama_rows_num,panorama_cols_num,3))
        src_image_after_backward[pad_struct[0]:dst_image.shape[0]+pad_struct[0],pad_struct[3]:pad_struct[3]+dst_image.shape[1]:] = dst_image

        panorama_img = np.clip(src_image_after_backward, 0, 255).astype(np.uint8)
